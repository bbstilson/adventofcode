...
class Day7Spec extends UnitSpec {
  ...
  "part 2 test" should "not regress" in {
    ...
    def a: LazyList[Int] = IntCodeComputer(program, 9 #:: 0 #:: e)
    def b: LazyList[Int] = IntCodeComputer(program, 8 #:: a)
    def c: LazyList[Int] = IntCodeComputer(program, 7 #:: b)
    def d: LazyList[Int] = IntCodeComputer(program, 6 #:: c)
    def e: LazyList[Int] = IntCodeComputer(program, 5 #:: d)

    e.last shouldBe 139629729
  }
  ...
  it should "not regress - 2" in {
    ...
    def a: LazyList[Int] = IntCodeComputer(program, 9 #:: 0 #:: e)
    def b: LazyList[Int] = IntCodeComputer(program, 7 #:: a)
    def c: LazyList[Int] = IntCodeComputer(program, 8 #:: b)
    def d: LazyList[Int] = IntCodeComputer(program, 5 #:: c)
    def e: LazyList[Int] = IntCodeComputer(program, 6 #:: d)

    e.last shouldBe 18216
  }
}
