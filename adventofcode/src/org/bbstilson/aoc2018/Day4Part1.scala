...
object Day4Part1 {
  ...
  def main(args: Array[String]): Unit = {
    val sortedAndParsed: Seq[LogLine] = io.Source
      .fromResource("2018/day4/input.txt")
      .getLines()
      .toList
      .sorted
      .map(parseLine _)
    ...
    val (sleepiestGuard, _) = guardMap.view
      .mapValues(_.values.sum)
      .toMap
      .toList
      .sortBy { case (_, minutesSlept) => minutesSlept }
      .last

    val (mostSleptMinute, _) = guardMap(sleepiestGuard).toList.sortBy { case (_, numHit) =>
      numHit
    }.last
