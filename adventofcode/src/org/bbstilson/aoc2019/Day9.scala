...
object Day9 {
  ...
  private def mkMemory(xs: List[Long]): Memory =
  ...
  private def paramMode(cmd: Long)(param: Int): Int =
    (cmd / math.pow(10, (2 + param).toDouble) % 10).toInt
  ...
  private def writeParamWithState(
    state: ProgramState,
  ...
  private def step(state: ProgramState): Option[(Option[Long], ProgramState)] = {
    val ProgramState(index, _, inputs, relativeBase) = state
    ...
    opcode match {
      ...
      case 9 => {
        Some(
          (None, state.copy(index = index + 2, relativeBase = relativeBase + readParam(0).toInt))
        )
      }
      case 99 => None
