...
object Part1 {
  ...
  def main(args: Array[String]): Unit = {
    val source: Seq[(Char, Char)] = Source
      .fromFile("input.txt")
      .getLines
      .toList
      .map(parse)
  ...
  private def calcInDegrees(g: Digraph): DegreeMap = {
    g.foldLeft(DegreeMap.initFromDigraph(g)) { case (degreeMap, (key, values)) => {
  ...
  private def topologicalSort(
    ...
    if (pQueue.isEmpty) {
      ...
      val nextDegreeMap = graph.get(letter) match {
        case Some(charSet) => charSet.foldLeft(degreeMap) { case (dMap, char) =>
          dMap(char) match {
            case 0 => dMap
      ...
      letter + topologicalSort(graph, nextDegreeMap, pQueue)
    }
  }

  private def getZeroInDegree(degreeMap: DegreeMap): Set[Char] = {
    degreeMap.filter { case (k, v) => v == 0 }.keySet
  }
}
