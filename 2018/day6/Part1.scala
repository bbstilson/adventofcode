...
object Part1 {
  ...
  private def mkVoronoiDiagram(
    ...
    .map { case (p, pointState) => (p, setPointState(sites, p)) }
  }

  private def setPointState(sites: Seq[Point], point: Point): PointState = {
    val (minDist, closestSites) = sites
      .map(site => (site, mDistance(site, point)))
      .groupBy({ case (site, dist) => dist })
  ...
  private def query(
    ...
    vDiagram
      .toList
      .map({ case (point, pointState) => pointState match {
        case Unique(site) => (point, Some(site))
        case _ => (point, None)
      }})
      .filter({ case (point, site) => site.isDefined })
      .map({ case (point, site) => (point, site.get) })
      .groupBy({ case (point, site) => site })
      .toList
      .map({ case (site, xs) => (site, xs.map { case (p, _) => p }) })
      // Filter out any sites that have at least one infinite point.
      .filterNot({ case (site, points) => points.exists(p => isInfinite(boundingBox, p)) })
      .map({ case (site, points) => (site, points.size) })
      .sortBy({ case (site, points) => points })
      .last
  }
